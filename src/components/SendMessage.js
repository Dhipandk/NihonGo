...
const SendMessage = ({ scroll, currentRoom, preferences }) => {
  ...
  const { transcript, listening, resetTranscript, browserSupportsSpeechRecognition } = useSpeechRecognition();
  const { uid, displayName, photoURL } = auth.currentUser;
  const [preferredLanguage, setPreferredLanguage] = useState(preferences.preferredLanguage || "en");
  const [translatedLanguage, setTranslatedLanguage] = useState(preferences.preferredLanguage === "en" ? "ja" : "en");
  ...
  const handleMicClick = async () => {
    try {
      const preferLanguage = preferredLanguage;
      ...
      console.error("Error with speech recognition:", error);
      alert("Please allow microphone access for using this feature.");
    }
  };
